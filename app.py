...
class LLMClient:
    ...
    def __init__(self, config):
        ...
        self.embedding_failed = False # 标记 Embedding 服务是否不可用
    ...
    def get_embedding(self, text):
        ...
        # 优先使用 text-embedding-004，如果失败用户可以在 UI 改
...
class VectorStore:
    ...
    def add_documents(self, file_corpus):
        ...
        if self.llm_client:
            with st.status("正在构建索引 (尝试向量化 + 关键词库)...") as status:
                ...
                # 即使 Embedding 失败，文档依然保留，用于关键词检索
                self.vectors = results # 包含 None 的列表
                
                if success_count == 0:
                    status.update(label="⚠️ 向量化全部失败 (将降级使用关键词检索)", state="error")
                    st.warning("提示：Embedding API 调用失败，已自动切换为 **关键词匹配模式**。请检查 API Key 是否支持 Embedding。")
                else:
    ...
    def search(self, query_text, top_k=3):
        ...
        if query_vec is not None and has_vectors:
            # 向量计算
            ...
            if norm_q > 0:
                ...
                for i, doc_vec in enumerate(self.vectors):
                    ...
                    if norm_d == 0:
                        ...
                        scores.append(np.dot(d_v, q_v) / (norm_d * norm_q))
        ...
        for doc in self.documents:
            # 计算简单的 Jaccard 相似度或重叠数
            overlap = sum(1 for k in query_keywords if k in doc['text'])
            if overlap > 0:
                # 归一化分数 (简单的)
                score = overlap / (len(query_keywords) + 1) * 0.8 # 关键词匹配最高给 0.8
                kw_results.append({
                    'doc': doc,
                    'score': score,
                    'method': 'keyword'
...
st.markdown("""
**核心技术**: `Hybrid Search` (向量 + 关键词) + `Map-Reduce` 并发架构。
即便 Embedding 接口不可用，系统也会自动降级为关键词匹配，确保永远有结果。
""")
    ...
    if not (reg_files and policy_files and llm_api_key):
        ...
        for name, content in process_uploaded_files(policy_files):
            ...
            if text and len(text.strip()) > 0:
                policy_corpus.append({'name': name, 'content': text})
            else:
                st.warning(f"文件 {name} 内容为空或提取失败，已跳过。" )
            
        if not policy_corpus:
            st.error("有效制度内容为空，请检查文件格式。" )
            st.stop()
        ...
        if not all_clauses:
             st.error("未解析出任何法规条款，请检查法规文件格式 (需包含'第X条')。" )
             st.stop()
...
if st.session_state.results is not None:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "匹配度": st.column_config.ProgressColumn("匹配度", min_value=0, max_value=1, format="%.2f")
        },
